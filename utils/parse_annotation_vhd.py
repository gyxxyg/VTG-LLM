...
with open(annotation_path, 'r') as f:
    for line in f:
        ...
        if len(matches) > 0:
            ...
            for match in matches:
                answer += f'{match[1]} seconds, significance score: {match[4]}, {match[-1]}'.strip()
                if answer[-1] != '.':
                    answer += '. '
                else:
                    answer += ' '
        ...
        if len(matches) > 0:
            answer = ''
            for match in matches:
                answer += f'{match[0]} seconds, significance score: {match[-2]}, {match[-1].strip()}'.strip()
                if answer[-1] != '.':
                    answer += '. '
                else:
                    answer += ' '
    ...
    #     matched_num = 0
        ...
        pattern = re.compile(r'(\d+[\.\d]*)\s*-\s*(\d+[\.\d]*)\s*(seconds?)?\s*,\s*([sS]ignificance\s*score)\s*:\s*(\d+)\s*,\s*([^{}"]*?)"')
        matches = pattern.findall(event_data)
        if len(matches) > 0:
            answer = ''
            for match in matches:
                # print(matches)
                timestamps = [float(match[0]) + (float(match[1]) - float(match[0])) * i / 10 for i in range(10)]
                answer = ''
                for time in timestamps:
                    answer += f'{time} seconds, significance score: {match[-2]}, {match[-1].strip()}'.strip()
                    if answer[-1] != '.':
                        answer += '. '
                    else:
                        answer += ' '
                ...
                dvc_data_item['QA'][0]['q'] = item_prompt.replace('<query_placeholder>', match[-1].strip())
                dvc_data_item['QA'][0]['a'] = answer.strip()
                data.append(dvc_data_item)
                print(dvc_data_item)
    ...
    out_file_path = output_path + f'vhd_{len(data) // 1000}k.json'
