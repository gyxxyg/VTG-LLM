...
if __name__ == "__main__":
    ...
    '''
    prompts = list(get_prompt("VTG-LLM/utils/construct_it_data/prompts/action_locate_prompts.json").values())
    ...
    for split in ["train"]:  # "val", "test"
        if dataset == "coin":
            ...
            filename = f"all_data_{split}.json"
            print(os.path.join(args.anno_path, filename))
            annos = read_json(os.path.join(args.anno_path, filename))
            it_data = []
            for jterm in annos:
