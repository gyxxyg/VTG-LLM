...
import decord
import numpy as np
from vtgllm.processors import transforms_video
from vtgllm.processors.base_processor import BaseProcessor
from vtgllm.processors.randaugment import VideoRandomAugment
from vtgllm.processors import functional_video as F
...
def load_video(video_path, n_frms=MAX_INT, height=-1, width=-1, sampling="uniform", return_msg = False):
    decord.bridge.set_bridge("torch")
    # print('begin load', video_path)

    # with warnings.catch_warnings(record=True) as w:
    #     warnings.filterwarnings("ignore")  # catch all warnings
    #     vr = VideoReader(uri=video_path, height=height, width=width)  # replace with your video processing code
        # if len(w) > 0:  # if there are any warnings
        #     for warning in w:
    ...
    # print('finished load', video_path)
    return frms, msg
