...
from vtgllm.models.blip2_qformer import Blip2Qformer
...
class Blip2ITM(Blip2Qformer):
    ...
    def forward(self, samples, match_head="itc"):
        ...
        with self.maybe_autocast():
            image_embeds = self.ln_vision(self.visual_encoder(image.unsqueeze(0).permute(0, 2, 1, 3, 4)))
        ...
        if match_head == "itm":
            ...
            # print(itm_logit.shape)
            itm_logit = itm_logit.mean(dim=1)

            return torch.softmax(itm_logit, dim=1)[:, 0]
